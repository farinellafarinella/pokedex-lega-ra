-- Corregge la verifica pubblica dei token Pokédex già installata nel database.
...
declare
 ...
 v_hash text:=encode(digest(coalesce(p_activation_token,''),'sha256'),'hex');
...
begin
 ...
 select * into v_device from public.pokedex_devices where upper(trim(activation_token))=upper(trim(p_activation_token));
 v_exists:=found;
