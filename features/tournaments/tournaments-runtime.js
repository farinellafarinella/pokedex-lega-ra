...
const emptyDraft=()=>({name:'',description:'',fee:'0',date:'',time:'',first:'',second:'',third:''});
const eventDate=t=>t.event_date?t.event_date.split('-').reverse().join('/'):'Data da definire';
const schedule=t=>`<div class="schedule"><span>Data<strong>${esc(eventDate(t))}</strong></span><span>Ora italiana<strong>${t.event_time?esc(t.event_time.slice(0,5)):'Da definire'}</strong></span></div>`;
const prizes=t=>[t.prize_first,t.prize_second,t.prize_third].some(p=>p?.trim())?`<section class="awards"><h3>Premi previsti</h3>${[t.prize_first,t.prize_second,t.prize_third].map((p,i)=>`<div><strong>${i+1}° posto</strong><p>${p?esc(p):'Non specificato'}</p></div>`).join('')}</section>`:'';
const money=value=>new Intl.NumberFormat('it-IT').format(Number(value)||0)+' ₽';
const messages={INVALID_EVENT_SCHEDULE:'Inserisci sia la data sia l’ora dell’evento, con precisione al minuto.',INVALID_EVENT_PRIZE:'Ogni premio può contenere al massimo 300 caratteri.',EVENT_CHANGED:'L’evento è cambiato nel frattempo. Premi Aggiorna e controlla lo stato prima di eliminarlo.',ACCOUNT_NOT_ACTIVE:'Accedi con una Scheda Allenatore attiva.',ADMIN_REQUIRED:'Questa operazione è riservata agli admin.',TOURNAMENT_NOT_FOUND:'Evento non trovato. Aggiorna la pagina.',INVALID_TOURNAMENT:'Inserisci un nome da 2 a 80 caratteri e una descrizione di massimo 600 caratteri.',REGISTRATION_CLOSED:'Le iscrizioni sono chiuse. Aggiorna l’evento.',TOURNAMENT_FULL:'L’evento ha raggiunto il limite di 128 iscritti.',NOT_ENOUGH_PLAYERS:'Servono almeno 2 iscritti per avviare l’evento.',INVALID_WINNER:'Seleziona uno dei due giocatori dell’incontro.',MATCH_NOT_READY:'Aspetta che siano decisi entrambi i giocatori di questo incontro.',RESULT_ALREADY_SET:'Il risultato è già stato confermato. Aggiorna il tabellone.',TOURNAMENT_NOT_RUNNING:'L’evento non è in corso.',OPERATION_CONFLICT:'Operazione già utilizzata. Riapri il modulo e riprova.'};
...
async function mountTournaments(host,{client,isCurrent=()=>true,initialId=null,onBalance=()=>{}}){
 ...
 root.innerHTML=`<link rel="stylesheet" href="${new URL('./style.css?v=4',scriptURL)}"><main id="view"></main>`;
 ...
 let creating=false,draft=emptyDraft(),createOperation=null,entryOperation=null;
 ...
 function detail(){const t=state.selected;if(!t)return `<p>Evento non trovato.</p>${button('back','Tutti gli eventi')}`;
  ...
  ${state.is_admin?`<div class="actions">${button('delete','Elimina evento','class="danger"')}</div>`:''}
 ...
 function listing(){return `${state.is_admin?`${button('new','+ Crea evento','class="primary"')}${creating?`<form id="create-form"><h2>Nuovo evento</h2><label>Nome<input name="name" required minlength="2" maxlength="80" value="${esc(draft.name)}" placeholder="Es. Coppa di Kanto" ${busy?'disabled':''}></label><div class="form-pair"><label>Data dell’evento<input name="date" type="date" required value="${esc(draft.date)}" ${busy?'disabled':''}></label><label>Ora dell’evento<input name="time" type="time" step="60" required value="${esc(draft.time)}" ${busy?'disabled':''}></label></div><p class="small">Ora italiana · L’avvio resta manuale tramite il pulsante admin.</p><label>Descrizione e regole<textarea name="description" maxlength="600" rows="4" placeholder="Luogo e regole degli incontri" ${busy?'disabled':''}>${esc(draft.description)}</textarea></label><label>Costo d’iscrizione (Pokédollari)<input name="fee" type="number" min="0" max="1000000" step="1" required value="${esc(draft.fee)}" ${busy?'disabled':''}></label><fieldset class="prize-fields"><legend>Premi per i primi tre classificati</legend>${[['first','1°'],['second','2°'],['third','3°']].map(([field,place])=>`<label>Premio ${place} posto<textarea name="${field}" rows="2" maxlength="300" placeholder="Descrivi il premio" ${busy?'disabled':''}>${esc(draft[field])}</textarea></label>`).join('')}<p class="small">Campi descrittivi: questi premi sono consegnati dall’organizzatore. Il montepremi in Pokédollari viene accreditato automaticamente al vincitore.</p></fieldset><p class="small">0 = gratuito. Tutto l’incasso va al vincitore. La quota non cambia dopo la creazione. Da 2 a 128 giocatori · Eliminazione diretta</p><div class="actions"><button type="submit" class="primary" ${busy?'disabled':''}>Apri le iscrizioni</button>${button('close-form','Annulla')}</div></form>`:''}`:''}${feedback()}
  ${['open','running','finished','cancelled'].map(status=>{const list=state.tournaments.filter(t=>t.status===status);return list.length?`<section><h2>${{open:'Iscriviti a un evento',running:'Eventi in corso',finished:'Eventi conclusi',cancelled:'Eventi annullati'}[status]}</h2><div class="tournament-list">${list.map(t=>`<article class="tournament"><span class="pill ${status}">${labels[status]}</span><h3>${esc(t.name)}</h3>${schedule(t)}<p>${t.participant_count} iscritti${t.joined?' · Sei iscritto':''}</p><p>Quota: <b>${money(t.entry_fee)}</b> · Montepremi: <b>${money(t.prize_pool)}</b></p>${t.winner_name?`<p class="joined">Vincitore: ${esc(t.winner_name)}</p>`:''}${button('open',status==='open'?'Iscrizioni e dettagli':'Vedi tabellone',`data-id="${t.id}"`)}</article>`).join('')}</div></section>`:'';}).join('')||'<p>Non ci sono ancora eventi. Quando un admin ne crea uno, potrai iscriverti qui.</p>'}`;}
 ...
 root.addEventListener('input',e=>{if(['name','description','fee','date','time','first','second','third'].includes(e.target.name)){draft[e.target.name]=e.target.value;createOperation=null;}});
 root.addEventListener('submit',e=>{if(e.target.id!=='create-form')return;e.preventDefault();if(busy)return;try{createOperation??=uuid();}catch(error){notice='Impossibile creare l’evento in questo browser. Apri il sito con un browser aggiornato.';render();return;}
  mutate('create_league_tournament',{p_id:createOperation,p_name:draft.name.trim(),p_description:draft.description,p_entry_fee:Number(draft.fee),p_event_date:draft.date,p_event_time:draft.time,p_prize_first:draft.first,p_prize_second:draft.second,p_prize_third:draft.third},'Evento creato. Le iscrizioni sono aperte.',()=>{selectedId=createOperation;creating=false;draft=emptyDraft();createOperation=null;});
 });
 root.addEventListener('click',e=>{const b=e.target.closest('button[data-action]');if(!b||b.disabled||busy||!active())return;const a=b.dataset.action;
  ...
  if(a==='close-form'){creating=false;draft=emptyDraft();createOperation=null;render();return;}
  if(a==='dismiss'){confirmAction=null;render();return;}
  if(['join','start','cancel','leave','winner','delete'].includes(a)){confirmAction={type:a,match:b.dataset.match,winner:b.dataset.winner};render();const commit=root.querySelector('[data-action=commit]');commit?.focus({preventScroll:true});commit?.scrollIntoView({block:'nearest'});return;}
  if(a==='commit'&&confirmAction){const c=confirmAction;
   if(c.type==='delete')return mutate('delete_league_tournament',{p_tournament_id:selectedId,p_expected_status:state.selected.status},'Evento eliminato.',()=>{selectedId=null;entryOperation=null;state={...state,tournaments:state.tournaments.filter(t=>t.id!==state.selected?.id),selected:null,entries:[],matches:[]};});
