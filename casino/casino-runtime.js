...
async function mountCasino(host,{client,api=createCasinoAPI(client),isCurrent=()=>true,onBalance=()=>{}}){
 ...
 const details=()=>diagnostic?`<details><summary>Dettagli dell’errore</summary><pre style="white-space:pre-wrap;overflow-wrap:anywhere">${esc(diagnostic)}</pre></details>`:'';
 ...
 function render(){if(!active())return;
  if(!state){view.innerHTML=`<h1>Casinò di Zafferanopoli</h1><p role="status">${esc(notice||'Caricamento dell’estrazione settimanale del venerdì…')}</p>${button('refresh','Riprova')}${diagnostic?`<details><summary>Dettagli dell’errore</summary><pre style="white-space:pre-wrap;overflow-wrap:anywhere">${esc(diagnostic)}</pre></details>`:''}`;return;}
  const current=state.current_draw,draw=state.selected_draw,entry=state.my_entry,own=state.my_selected_entry,open=playable(),friday=!!current;
  view.innerHTML=`<div class="eyebrow">ESTRAZIONE SETTIMANALE DEL VENERDÌ</div><h1>Casinò di Zafferanopoli</h1><p>Casinò consultabile tutti i giorni. Tre carte su dieci, con un’unica estrazione ogni venerdì alle 20:00.</p><div class="balance">Saldo <strong>${money(state.balance)}</strong></div><p id="notice" role="status" aria-live="polite">${esc(notice)}</p><article class="schedule"><span class="pill">${current?.status==='open'?'GIOCATE APERTE · ESTRAZIONE VENERDÌ ALLE 20:00':current?.status==='drawn'?'ESTRAZIONE CONCLUSA':'GIOCATE CHIUSE'}</span><h2>${current?.status==='open'?'Estrazione di venerdì '+date(current.draw_date+'T12:00:00Z'):'Le giocate riaprono venerdì'}</h2><p>${current?.status==='open'?'Scegli le carte in qualsiasi giorno e conferma entro venerdì alle 19:59:59. Una giocata per estrazione, senza modifiche dopo la conferma.':'Il Casinò è sempre consultabile: scopri risultati, storico e classifiche. Puoi confermare le carte tutta la settimana per il prossimo venerdì.'}</p><strong id="countdown" aria-live="off"></strong><small id="countdown-label"></small></article>
  ${current?.status==='open'?`<article><div class="select-head"><h2>${entry?'Giocata confermata':'Scegli esattamente 3 carte'}</h2><span>${selected.size} / 3</span></div><div class="card-grid">${Array.from({length:10},(_,i)=>i+1).map(n=>button('card',`<small>CARTA</small><b>${n}</b><span>◓</span>${cardImage(n)}`,`class="playing-card ${selected.has(n)?'selected':''}" data-card="${n}" aria-label="Carta ${n}" aria-pressed="${selected.has(n)}" ${!open||busy?'disabled':''}`)).join('')}</div><div id="confirmation-status" role="status" aria-live="polite">${esc(notice)}</div>${details()}${entry?'<p class="locked">✓ Carte confermate. Non puoi più modificarle.</p>':`<p>Partecipazione gratuita · La selezione diventa definitiva solo dopo la conferma.</p>${confirmation?`<div class="confirm"><p>Confermi le carte <b>${[...selected].sort((a,b)=>a-b).join(', ')}</b>? La giocata non potrà essere modificata.</p>${button('send','Conferma definitiva',`class="primary" ${!open||busy?'disabled':''}`)}${button('cancel','Rivedi le carte',busy?'disabled':'')}</div>`:button('confirm','Conferma le 3 carte',`class="primary" ${selected.size!==3||!open||busy?'disabled':''}`)}`}</article>`:''}
 ...
 function updateCountdown(){if(!state||!active())return;const current=state.current_draw,until=current?.status==='open'?Date.parse(current.closes_at):Date.parse(state.next_open);let seconds=Math.max(0,Math.ceil((until-serverTime())/1000));
  ...
  const label=root.querySelector('#countdown-label');if(label)label.textContent=current?.status==='open'?'All’estrazione del venerdì · Europe/Rome':'Al prossimo venerdì · Europe/Rome';
 ...
 root.addEventListener('click',async e=>{const b=e.target.closest('button');if(!b||b.disabled||busy||!active())return;const a=b.dataset.action;
  ...
  if(a==='confirm'){if(playable()&&selected.size===3){confirmation=true;render();const send=root.querySelector('[data-action=send]');send?.focus({preventScroll:true});send?.scrollIntoView({block:'nearest'});}return;}
  ...
  if(a==='send'){
   ...
   busy=true;diagnostic='';notice='Conferma in corso…';render();
   try{if(!pending||pending.key!==key)pending={key,id:operationId()};const data=await api.submit(choices,pending.id,state.current_draw.draw_date);if(!active())return;accept(data);pending=null;confirmation=false;notice=data.error_code?errors[data.error_code]:'Giocata confermata. Le carte non sono più modificabili.';}
   catch(error){console.error('Conferma Casinò:',error);diagnostic=[error?.code,error?.message||String(error),error?.details,error?.hint].filter(Boolean).join('\n');notice=errorText(error);try{const data=await api.read(selectedDate);if(active()){accept(data);if(data.my_entry){pending=null;confirmation=false;diagnostic='';notice='Giocata confermata. Le carte non sono più modificabili.';}}}catch{} }
